...
namespace SharpLua
{
    ...
    public class LuaRuntime
    {
        ...
        public static LuaValue RunFile(string luaFile)
        {
            ...
            return Run(File.ReadAllText<LuaRuntime>(luaFile));
        }
        ...
        {
            luaFile = FindFullPath(luaFile);
            return Run(File.ReadAllText<LuaRuntime>(luaFile), enviroment);
        }
                                                      ...
                                                      bool createClassLib = true)
        {
            ...
            global.SetNameValue("_WORKDIR", new LuaString("\\"));
        ...
        {
            if (File.Exists<LuaRuntime>(spath))
                return spath;
            if (File.Exists<LuaRuntime>(spath + ".lua")) // lua
                return spath + ".lua";
            if (File.Exists<LuaRuntime>(spath + ".wlua"))
                return spath + ".wlua"; // wLua
            if (File.Exists<LuaRuntime>(spath + ".slua")) // sLua (SharpLua)
                return spath + ".slua";
            if (File.Exists<LuaRuntime>(spath + ".dll"))
                return spath + ".dll";
            if (File.Exists<LuaRuntime>(spath + ".exe"))
                return spath + ".exe";
        ...
        private static void LoadFiles()
        {
            ...
            {
                A8Console.WriteLine("Error loading startup script 'init': " + e.ToString());
            }
            try {
                // check startup script
            ...
            {
                A8Console.WriteLine("Error loading startup script 'boot': " + e.ToString());
            }
            try {
                // check startup script
            ...
            {
                A8Console.WriteLine("Error loading startup script 'start': " + e.ToString());
            }
            */
        }
