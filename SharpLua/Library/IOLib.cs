...
namespace SharpLua.Library
{
    public static class IOLib
    {
        ...
        private static TextReader DefaultInput = null;
        private static TextWriter DefaultOutput = null;
        ...
        public static LuaValue Output(LuaValue[] values)
        {
            ...
            else
            {
                ...
                if (file != null)
                {
                    DefaultOutput = File.CreateText<LuaRuntime>(file.Text);
                    return null;
                }
        ...
        public static LuaValue Open(LuaValue[] values)
        {
            ...
            switch (mode)
            {
                case "r":
                case "r+":
                    TextReader reader = File.OpenText<LuaRuntime>(file.Text);
                    return new LuaUserdata(reader, FileLib.CreateMetaTable());
                case "w":
                case "w+":
                    TextWriter writer = File.CreateText<LuaRuntime>(file.Text);
                    return new LuaUserdata(writer, FileLib.CreateMetaTable());
                case "a":
                case "a+":
                    writer = File.AppendText<LuaRuntime>(file.Text);
                    return new LuaUserdata(writer, FileLib.CreateMetaTable());
                default:
        ...
        {
            Random r = new Random();
            TextWriter writer = File.CreateText<LuaRuntime>(OSLib.TmpName(null).ToString());
        ...
        private static LuaValue GetLines(LuaValue[] args)
        {
            ...
            if (args.Length > 0)
            {
                string[] lines = File.ReadAllLines<LuaRuntime>((args[0] as LuaString).Text);
